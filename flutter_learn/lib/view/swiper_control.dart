...
class FixSwiperControl extends SwiperPlugin {
  ...
  Widget buildButton(SwiperPluginConfig config, Color color, IconData iconDaga,
    ...
    return new GestureDetector(
      ...
      onTap: () {
        if (previous) {
          if (config.loop || config.activeIndex > 0) {
            config.controller.previous(animation: true);
          }
        } else {
          if (config.loop || config.activeIndex < config.itemCount - 1) {
